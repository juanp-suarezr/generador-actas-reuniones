...
// src/useFormState.ts
import { router as router2, UseFormUtils } from "@inertiajs/core";
...
function useFormState(options) {
    ...
    const form = reactive({
        ...
        withPrecognition(...args) {
            precognitionEndpoint = UseFormUtils.createWayfinderCallback(
                ...args,
            );
            ...
            Object.assign(formWithPrecognition, {
                ...
                validator: () => validator,
                withAllErrors: () =>
                    tap(formWithPrecognition, () => (withAllErrors = true)),
                ...
                validateFiles: () =>
                    tap(formWithPrecognition, () => validator.validateFiles()),
                ...
                setErrors: (errors) =>
                    tap(formWithPrecognition, () => this.setError(errors)),
                forgetError: (field) =>
                    tap(formWithPrecognition, () =>
            ...
            if (typeof fieldOrFields === "undefined") {
                ...
                defaults =
                    typeof fieldOrFields === "string"
                        ? set(cloneDeep2(defaults), fieldOrFields, maybeValue)
                        : Object.assign(
                              {},
                              cloneDeep2(defaults),
                              fieldOrFields,
        ...
        reset(...fields) {
            const resolvedData = isDataFunction
                ? cloneDeep2(resolveData())
                : defaults;
...
function validateFormDataKeys(data) {
    ...
    const conflicts = Object.keys(data).filter((key2) =>
        reservedFormKeys.has(key2),
    );
...
function useForm(...args) {
    const { rememberKey, data, precognitionEndpoint } =
        UseFormUtils2.parseUseFormArguments(...args);
    const initialDefaults =
        typeof data === "function" ? cloneDeep3(data()) : cloneDeep3(data);
    ...
    Object.assign(form, {
        submit(...args2) {
            const { method, url, options } = UseFormUtils2.parseSubmitArguments(
                args2,
                getPrecognitionEndpoint(),
            );
            resetDefaultsCalledInOnSuccess();
            ...
            _options.optimistic =
                _options.optimistic ?? pendingOptimisticCallback ?? void 0;
        ...
        return () => {
            if (component.value) {
                ...
                if (
                    ...
                    isRenderFunction(component.value.layout)
                ) {
                ...
                if (
                    ...
                    layoutValue.length <= 1 &&
                    typeof layoutValue.prototype === "undefined"
                ) {
                    ...
                    effectiveLayout =
                        layoutValue ??
                        defaultLayout?.(page.value.component, page.value);
                }
                if (effectiveLayout) {
                    let layouts = normalizeLayouts(
                        effectiveLayout,
                        isComponent,
                        component.value.layout && !callbackProps
                            ? isRenderFunction
                            : void 0,
                    );
                    if (callbackProps) {
                        ...
                        return layouts.reduceRight((childNode, layout2) => {
                            ...
                            return h(
                                layoutComponent,
                                {
                                    ...
                                    ...dynamicProps.shared,
                                    ...(layout2.name
                                        ? dynamicProps.named[layout2.name] || {}
                                        : {}),
                                },
                                () => childNode,
                            );
...
async function createInertiaApp({
    ...
    const isServer = typeof window === "undefined";
    const resolveComponent = (name, page3) =>
        Promise.resolve(resolve(name, page3)).then(
            (module) => module.default || module,
        );
    if (isServer && !page2 && !render) {
        return async (page3, renderToString) => {
            let head2 = [];
            const initialComponent = await resolveComponent(
                page3.component,
                page3,
            );
            const props = {
            ...
            if (setup) {
                ...
                vueApp2 = createSSRApp({
                    render: () => h2(app_default, props),
                });
                vueApp2.use(plugin);
                if (withApp) {
...
var deferred_default = defineComponent2({
    ...
    setup(props, { slots }) {
        ...
        return () => {
            if (!slots.fallback) {
                throw new Error(
                    "`<Deferred>` requires a `<template #fallback>` slot",
                );
            }
            const propsAreDefined = keys.value.every(
                (key2) => get2(page2.props, key2) !== void 0,
            );
            const hasRescuedProps = keys.value.some((key2) =>
                rescuedKeys.value.has(key2),
            );
...
var Form = defineComponent3({
    ...
    setup(props, { slots, attrs, expose }) {
        ...
        const onFormUpdate = (event) => {
            if (
                event.type === "reset" &&
                event.detail?.[FormComponentResetSymbol]
            ) {
            ...
            isDirty.value =
                event.type === "reset"
                    ? false
                    : !isEqual2(getData(), formDataToObject(defaultData.value));
        };
        ...
        watch2(
            () => props.validateFiles,
            (value) =>
                value ? form.validateFiles() : form.withoutFileValidation(),
        );
        watch2(
        ...
        onBeforeUnmount(() =>
            formEvents.forEach((e) =>
                formElement.value?.removeEventListener(e, onFormUpdate),
            ),
        );
        const getFormData = (submitter) =>
            new FormData(formElement.value, submitter);
        ...
        const submit = (submitter) => {
            ...
            const submitOptions = {
                ...
                optimistic: props.optimistic
                    ? (pageProps) => props.optimistic(pageProps, data)
                    : void 0,
            ...
            form.transform(() => props.transform(data)).submit(
                method.value,
                url,
                submitOptions,
            );
        ...
        const exposed = {
            ...
            validate: (field, config3) =>
                form.validate(
                    ...UseFormUtils3.mergeHeadersForValidation(
                        field,
                        config3,
                        props.headers,
                    ),
...
var Head = defineComponent4({
    ...
    setup(props, { slots }) {
        ...
        return () => {
            provider.update(
                renderNodes(slots.default ? slots.default() : [], props.title),
            );
        };
    },
});
...
var InfiniteScroll = defineComponent5({
    ...
    setup(props, { slots, attrs, expose }) {
        ...
        const scrollableParent = computed4(() =>
        ...
        const manualMode = computed4(
            () =>
                props.manual ||
                (props.manualAfter > 0 &&
                    requestCount.value >= props.manualAfter),
        );
...
function useHttp(...args) {
    const { rememberKey, data, precognitionEndpoint } =
        UseFormUtils4.parseUseFormArguments(...args);
    ...
    const submit = async (method, url, options) => {
        ...
        options.optimistic =
            options.optimistic ?? pendingOptimisticCallback ?? void 0;
        pendingOptimisticCallback = null;
        let snapshot;
        ...
        try {
            ...
            if (error instanceof HttpResponseError) {
                if (error.response.status === 422) {
                    ...
                    const processedErrors = withAllErrors.enabled()
                        ? validationErrors
                        : toSimpleValidationErrors2(validationErrors);
    ...
    Object.assign(form, {
        submit(...args2) {
            const parsed = UseFormUtils4.parseSubmitArguments(
                args2,
                getPrecognitionEndpoint(),
            );
...
function usePrefetch(options = {}) {
    ...
    const cached =
        typeof window === "undefined"
            ? null
            : router9.getCached(window.location.pathname, options);
    const inFlight =
        typeof window === "undefined"
            ? null
            : router9.getPrefetching(window.location.pathname, options);
...
function useRemember(data, key2) {
    ...
    const remembered = type(
        restored === void 0
            ? data
            : hasCallbacks
              ? data.__restore(cloneDeep5(restored))
              : cloneDeep5(restored),
    );
    watch4(
        remembered,
        (newValue) => {
            router10.remember(
                cloneDeep5(hasCallbacks ? data.__remember() : newValue),
                key2,
            );
        },
        { immediate: true, deep: true },
...
var whenVisible_default = defineComponent7({
    ...
    setup(props, { slots }) {
        ...
        function getReloadParams() {
            ...
            if (props.data) {
                reloadParams.only = Array.isArray(props.data)
                    ? props.data
                    : [props.data];
            }
            return reloadParams;
        ...
        watch5(
            ...
            () => {
                const exists =
                    keys.value.length > 0 &&
                    keys.value.every(
                        (key2) => get3(page2.props, key2) !== void 0,
                    );
                loaded.value = exists;

...
var firePrefetchedEvent = (response, visit) => {
    return fireEvent("prefetched", {
        detail: { fetchedAt: Date.now(), response, visit },
    });
};
...
var PrefetchedRequests = class {
    ...
    add(params, sendFunc, { cacheFor, cacheTags }) {
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            this.scheduleForRemoval(
                params,
                oncePropExpiresIn
                    ? Math.min(prefetchExpiresIn, oncePropExpiresIn)
                    : prefetchExpiresIn,
            );
    ...
    clearTimer(params) {
        ...
        if (timer) {
            ...
            this.removalTimers = this.removalTimers.filter(
                (removalTimer) => removalTimer !== timer,
            );
        }
    }
    ...
    updateCachedOncePropsFromCurrentPage() {
        this.cached.forEach((prefetched) => {
            prefetched.response.then((response) => {
                ...
                for (const [group, deferredProps] of Object.entries(
                    pageResponse.deferredProps ?? {},
                )) {
                    const remaining = deferredProps.filter(
                        (prop) => get2(pageResponse.props, prop) === void 0,
                    );
                ...
                const oncePropExpiresIn =
                    this.getShortestOncePropTtl(pageResponse);
    ...
    getShortestOncePropTtl(page2) {
        const expiryTimestamps = Object.values(page2.onceProps ?? {})
...
var getScrollableParent = (element) => {
    const allowsVerticalScroll = (el) => {
        ...
        return (
            hasDimensionConstraint(computedStyle.maxHeight, el.style.height) ||
            isConstrainedByLayout(el, "height")
        );
    };
    const allowsHorizontalScroll = (el) => {
        ...
        return (
            hasDimensionConstraint(computedStyle.maxWidth, el.style.width) ||
            isConstrainedByLayout(el, "width")
        );
    };
    const hasDimensionConstraint = (
        computedMaxDimension,
        inlineStyleDimension,
    ) => {
        if (
            computedMaxDimension &&
            computedMaxDimension !== "none" &&
            computedMaxDimension !== "0px"
        ) {
            return true;
        }
        if (
            inlineStyleDimension &&
            inlineStyleDimension !== "auto" &&
            inlineStyleDimension !== "0"
        ) {
            return true;
        }
        return false;
    ...
    const isConstrainedByLayout = (el, dimension) => {
        ...
        if (["flex", "inline-flex"].includes(parentStyle.display)) {
            const isColumnLayout = ["column", "column-reverse"].includes(
    ...
    while (parent) {
        const allowsScroll =
            allowsVerticalScroll(parent) || allowsHorizontalScroll(parent);
        if (
...
var Scroll = class {
    ...
    static onScroll(event) {
        ...
        if (
            ...
            target.hasAttribute("scroll-region")
        ) {
            this.save();
        }
    }
...
function objectToFormData(
    ...
    for (const key in source) {
        if (Object.prototype.hasOwnProperty.call(source, key)) {
            append(
                form,
                composeKey(parentKey, key, "indices"),
                source[key],
...
function append(form, key, value, format) {
    if (Array.isArray(value)) {
        return Array.from(value.keys()).forEach((index) =>
            append(
                form,
                composeKey(key, index.toString(), format),
                value[index],
                format,
            ),
        );
...
function hrefToUrl(href) {
    return new URL(
        href.toString(),
        typeof window === "undefined" ? void 0 : window.location.toString(),
    );
}
var transformUrlAndData = (
    href,
    data,
    method,
    forceFormData,
...
function mergeDataIntoQueryString(
    method,
    href,
    data,
    qsArrayFormat = "brackets",
) {
    const hasDataForQueryString =
        method === "get" && !isFormData(data) && Object.keys(data).length > 0;
    const hasHost = urlHasProtocol(href.toString());
    const hasAbsolutePath =
        hasHost || href.toString().startsWith("/") || href.toString() === "";
    const hasRelativePath =
        !hasAbsolutePath &&
        !href.toString().startsWith("#") &&
        !href.toString().startsWith("?");
    const hasRelativePathWithDotPrefix = /^[.]{1,2}([/]|$)/.test(
        href.toString(),
    );
    ...
    return [
        [
            ...
            hasRelativePath
                ? url.pathname.substring(hasRelativePathWithDotPrefix ? 0 : 1)
                : "",
...
var setHashIfSameUrl = (originUrl, destinationUrl) => {
    if (
        originUrl.hash &&
        !destinationUrl.hash &&
        urlWithoutHash(originUrl).href === destinationUrl.href
    ) {
...
var CurrentPage = class {
    ...
    set(
        ...
        return this.resolve(page2.component, page2).then((component) => {
            ...
            const scrollRegions =
                !isServer3 && preserveScroll ? Scroll.getScrollRegions() : [];
            replace =
                replace || isSameUrlWithoutHash(hrefToUrl(page2.url), location);
            const pageForHistory = { ...page2, flash: {} };
            return new Promise((resolve) =>
                replace
                    ? history.replaceState(pageForHistory, resolve)
                    : history.pushState(pageForHistory, resolve),
            ).then(() => {
                const isNewComponent = !this.isTheSame(page2);
                if (
                    !isNewComponent &&
                    Object.keys(page2.props.errors || {}).length > 0
                ) {
                    viewTransition = false;
                }
                this.page = page2;
                ...
                return this.swap({
                    ...
                    if (preserveScroll) {
                        window.requestAnimationFrame(() =>
                            Scroll.restoreScrollRegions(scrollRegions),
                        );
                    } else {
                        Scroll.reset();
                    }
                    if (
                        ...
                        this.pendingDeferredProps.component ===
                            page2.component &&
                        this.pendingDeferredProps.url === page2.url
                    ) {
                        eventHandler.fireInternalEvent(
                            "loadDeferredProps",
    ...
    setPropsQuietly(props) {
        ...
        return this.resolve(this.page.component, this.page).then(
            (component) => {
                return this.swap({
                    component,
                    page: this.page,
    ...
    swap({ component, page: page2, preserveState, viewTransition }) {
        const doSwap = () =>
            this.swapComponent({ component, page: page2, preserveState });
        if (
            !viewTransition ||
        ...
        const viewTransitionCallback =
            typeof viewTransition === "boolean" ? () => null : viewTransition;
        return new Promise((resolve) => {
            const transitionResult = document.startViewTransition(() =>
                doSwap().then(resolve),
            );
        ...
        Object.entries(response.onceProps ?? {}).forEach(([key, onceProp]) => {
            ...
            if (force || get3(response.props, onceProp.prop) === void 0) {
                set3(
                    response.props,
                    onceProp.prop,
                    get3(this.page.props, existingOnceProp.prop),
                );
...
var History = class {
    ...
    restore(key) {
        if (!isServer2) {
            return this.current[this.rememberedState]?.[key] !== void 0
    ...
    pushState(page2, cb = null) {
        ...
        queue.add(() => {
            return this.getPageData(page2).then((data) => {
                const doPush = () =>
                    this.doPushState({ page: data }, page2.url).then(() =>
                        cb?.(),
                    );
                if (isChromeIOS) {
                    return new Promise((resolve) => {
    ...
    getPageData(page2) {
        ...
        return new Promise((resolve) => {
            return page2.encryptHistory
                ? encryptHistory(pageWithClonedProps).then(resolve)
                : resolve(pageWithClonedProps);
        });
    }
    processQueue() {
    ...
    decryptPageData(pageData) {
        return pageData instanceof ArrayBuffer
            ? decryptHistory(pageData)
    ...
    replaceState(page2, cb = null) {
        ...
        queue.add(() => {
            return this.getPageData(page2).then((data) => {
                const doReplace = () =>
                    this.doReplaceState({ page: data }, page2.url).then(() =>
                        cb?.(),
                    );
                if (isChromeIOS) {
                    return new Promise((resolve) => {
    ...
    isHistoryThrottleError(error) {
        return (
            ...
            (error.message.includes("history.pushState") ||
                error.message.includes("history.replaceState"))
        );
    }
...
var EventHandler = class {
    ...
    init() {
        if (typeof window !== "undefined") {
            ...
            window.addEventListener(
                "scroll",
                debounce(Scroll.onWindowScroll.bind(Scroll), 100),
                true,
            );
        }
        if (typeof document !== "undefined") {
            document.addEventListener(
                "scroll",
                debounce(Scroll.onScroll.bind(Scroll), 100),
                true,
            );
        }
    }
        ...
        history
            ...
            .then((data) => {
                ...
                page.setQuietly(data, { preserveState: false }).then(() => {
                    ...
                    for (const [group, props] of Object.entries(
                        data.initialDeferredProps ?? data.deferredProps ?? {},
                    )) {
                        const missing = props.filter(
                            (prop) => get4(pageProps, prop) === void 0,
                        );
                        if (missing.length > 0) {
...
var InitialVisit = class {
    ...
    static handleBackForward() {
        if (
            !navigationType.isBackForward() ||
            !history.browserHasHistoryEntry()
        ) {
            return false;
        }
    ...
    static handleLocation() {
        ...
        const locationVisit =
            SessionStorage.get(SessionStorage.locationVisitKey) || {};
        ...
        history
            .decrypt(page.get())
            .then(() => {
                const rememberedState = history.getState(
        ...
        document.addEventListener(
            ...
            () => {
                this.polls.forEach((poll) =>
                    poll.isInBackground(document.hidden),
                );
            },
            false,
        );
...
function parseHeaders(xhr) {
    ...
    xhr.getAllResponseHeaders()
        ...
        .forEach((line) => {
            ...
            if (index > 0) {
                headers[line.slice(0, index).toLowerCase().trim()] = line
                    .slice(index + 1)
                    .trim();
            }
        });
        ...
        return new Promise((resolve, reject) => {
            ...
            if (config2.onUploadProgress) {
                xhr.upload.onprogress = (event) => {
                    const progress3 = event.lengthComputable
            ...
            xhr.onerror = () =>
                reject(new HttpNetworkError("Network error", config2.url));
            xhr.onload = () => {
                const response = {
                    status: xhr.status,
...
var http = {
    ...
    setClient(clientOrOptions) {
        ...
        if (clientOrOptions.xsrfCookieName) {
            precognitionClient.withXsrfCookieName(
                clientOrOptions.xsrfCookieName,
            );
        }
        if (clientOrOptions.xsrfHeaderName) {
            precognitionClient.withXsrfHeaderName(
                clientOrOptions.xsrfHeaderName,
            );
        }
    },
    /**
...
var RequestParams = class _RequestParams {
    ...
    setPreserveOptions(page2) {
        this.params.preserveScroll = _RequestParams.resolvePreserveOption(
            this.params.preserveScroll,
            page2,
        );
        this.params.preserveState = _RequestParams.resolvePreserveOption(
...
var partialReloadRequestsProp = (visit, prop) => {
    ...
    if (
        only.length > 0 &&
        !only.some((candidate) => isPathOrSubPath(prop, candidate))
    ) {
        return false;
    }
    if (
        except.length > 0 &&
        except.some((candidate) => isPathOrSubPath(prop, candidate))
    ) {
        return false;
    }
    return true;
...
var Response = class _Response {
    ...
    async handlePrefetch() {
        if (
            isSameUrlWithoutHash(this.requestParams.all().url, window.location)
        ) {
            this.handle();
        }
    }
    ...
    locationVisit(url) {
        try {
            SessionStorage.set(SessionStorage.locationVisitKey, {
                preserveScroll:
                    this.requestParams.all().preserveScroll === true,
            });
            if (typeof window === "undefined") {
                return;
            }
    ...
    async setPage() {
        ...
        pageResponse.url = history.preserveUrl
            ? page.get().url
            : this.pageUrl(pageResponse);
    ...
    shouldSetPage(pageResponse) {
        ...
        return (
            originatingUrl.origin === currentPageUrl.origin &&
            originatingUrl.pathname === currentPageUrl.pathname
        );
    }
    ...
    mergeProps(pageResponse) {
        if (
            ...
            pageResponse.component !== page.get().component
        ) {
            return;
        }
        ...
        propsToDeepMerge.forEach((prop) => {
            ...
            set4(
                pageResponse.props,
                prop,
                deepMerge(currentProp, incomingProp, prop),
            );
        });
        ...
        for (const key of nestedTopKeys) {
            ...
            if (
                ...
                this.isObject(pageResponse.props[key])
            ) {
                pageResponse.props[key] = this.deepMergeObjects(
                    currentValue,
                    pageResponse.props[key],
                );
            }
        }
        ...
        if (
            currentOriginalDeferred &&
            Object.keys(currentOriginalDeferred).length > 0
        ) {
    ...
    mergeRescuedProps(pageResponse) {
        ...
        const newRescued = new Set(
            currentRescued.filter(
                (prop) =>
                    !partialReloadRequestsProp(this.requestParams.all(), prop),
            ),
        );
    ...
    mergeOrMatchItems(
        ...
        return shouldAppend
            ? this.appendWithMatching(
                  items,
                  newItems,
                  newItemsMap,
                  uniqueProperty,
              )
            : this.prependWithMatching(
                  items,
                  newItems,
                  newItemsMap,
                  uniqueProperty,
    ...
    appendWithMatching(existingItems, newItems, newItemsMap, uniqueProperty) {
        const updatedExisting = existingItems.map((item) => {
            if (
                this.hasUniqueProperty(item, uniqueProperty) &&
                newItemsMap.has(item[uniqueProperty])
            ) {
        ...
        const newItemsToAdd = newItems.filter((item) => {
            ...
            return !existingItems.some(
                (existing) =>
                    this.hasUniqueProperty(existing, uniqueProperty) &&
                    existing[uniqueProperty] === item[uniqueProperty],
            );
        });
    ...
    async setRememberedState(pageResponse) {
        const rememberedState = await history.getState(
            history.rememberedState,
            {},
        );
        if (
...
var Request = class _Request {
    ...
    getHeaders() {
        ...
        const onceProps = Object.entries(page2.onceProps || {})
            .filter(([, onceProp]) => {
...
var Router = class {
    ...
    poll(interval, requestOptions = {}, options = {}) {
        return polls.add(
            interval,
            () => this.reload({ preserveErrors: true, ...requestOptions }),
            {
    ...
    visit(href, options = {}) {
        options.optimistic =
            options.optimistic ?? this.pendingOptimisticCallback;
        ...
        const visit = this.getPendingVisit(href, {
            ...
            showProgress:
                ...
                (!options.async || !!options.optimistic),
        });
        ...
        const sendRequest = () => {
            ...
            if (prefetched) {
                ...
                const requestStream = visit.async
                    ? this.asyncRequestStream
                    : this.syncRequestStream;
                requestStream.send(
                    Request.create(requestParams, page.get(), {
    ...
    getCached(href, options = {}) {
        return prefetchedRequests.findCached(
            this.getPrefetchParams(href, options),
        );
    }
    flush(href, options = {}) {
    ...
    getPrefetching(href, options = {}) {
        return prefetchedRequests.findInFlight(
            this.getPrefetchParams(href, options),
        );
    }
    prefetch(href, options = {}, prefetchOptions = {}) {
        const method =
            options.method ?? (isUrlMethodPair(href) ? href.method : "get");
        if (method !== "get") {
        ...
        const visitUrl =
            visit.url.origin + visit.url.pathname + visit.url.search;
        const currentUrl =
        ...
        ensureCurrentPageIsSet().then(() => {
            prefetchedRequests.add(
                requestParams,
                (params) => {
                    this.asyncRequestStream.send(
                        Request.create(params, page.get()),
                    );
                },
                {
            ...
            props(currentProps) {
                const newValue =
                    typeof value === "function"
                        ? value(get7(currentProps, name), currentProps)
                        : value;
    ...
    appendToProp(name, value, options) {
        this.replaceProp(
            ...
            (currentValue, currentProps) => {
                const newValue =
                    typeof value === "function"
                        ? value(currentValue, currentProps)
                        : value;
    ...
    clientVisit(params, { replace = false } = {}) {
        this.clientVisitQueue.add(() =>
            this.performClientVisit(params, { replace }),
        );
    }
    performClientVisit(params, { replace = false } = {}) {
        ...
        const props =
            typeof params.props === "function"
                ? params.props(current.props, onceProps)
                : (params.props ?? current.props);
        const flash =
            typeof params.flash === "function"
                ? params.flash(current.flash)
                : params.flash;
        const {
            viewTransition,
            onError,
        ...
        const preserveScroll = RequestParams.resolvePreserveOption(
            params.preserveScroll ?? false,
            page2,
        );
        const preserveState = RequestParams.resolvePreserveOption(
        ...
        return page
            ...
            .then(() => {
                ...
                const scopedErrors = params.errorBag
                    ? errors[params.errorBag || ""] || {}
                    : errors;
    ...
    performInstantSwap(visit) {
        ...
        const sharedProps = Object.fromEntries(
            (current.sharedProps ?? [])
                .filter((key) => key in current.props)
                .map((key) => [key, current.props[key]]),
        );
        const resolvedPageProps =
            typeof visit.pageProps === "function"
                ? visit.pageProps(
                      cloneDeep4(current.props),
                      cloneDeep4(sharedProps),
                  )
                : visit.pageProps;
        const intermediateProps =
            resolvedPageProps !== null
                ? { ...resolvedPageProps }
                : { ...sharedProps };
        ...
        return page.set(intermediatePage, {
            ...
            preserveScroll: RequestParams.resolvePreserveOption(
...
function toHttpProgressEvent(axiosEvent) {
    return {
        ...
        percentage: axiosEvent.progress
            ? Math.round(axiosEvent.progress * 100)
            : 0,
...
var AxiosHttpClient = class {
    ...
    constructor(instance) {
        this.axiosInstance = instance
            ...
            : loadAxiosModule().then((module) => module.default);
    }
    async getAxios() {
    ...
    async doRequest(config2) {
        ...
        try {
            const response = await axios({
                ...
                responseType: "text",
                onUploadProgress: config2.onUploadProgress
                    ? (event) =>
                          config2.onUploadProgress(toHttpProgressEvent(event))
                    : void 0,
            });
            return {
        ...
        if (path[path.length - 1] === "") {
            ...
            if (Array.isArray(existing)) {
                ...
                set6(
                    ...
                    arrayPath,
                    numericKeys.length > 0
                        ? [...numericKeys.map((k) => existing[k]), value]
                        : [value],
                );
            } else {
...
var Renderer = {
    ...
    isInertiaManagedElement(element) {
        return (
            ...
            element.getAttribute("data-inertia") !== null
        );
    },
    findMatchingElementIndex(element, elements) {
        ...
        if (key !== null) {
            return elements.findIndex(
                (element2) => element2.getAttribute("data-inertia") === key,
            );
        }
        return -1;
    },
    update: debounce(function (elements) {
        ...
        if (
            sourceElements.some(
                (element) => element instanceof HTMLTitleElement,
            )
        ) {
            document.head
...
function createHeadManager(isServer3, titleCallback, onUpdate) {
    ...
    function collect() {
        ...
        const defaults = {
            ...(title
                ? { title: `<title data-inertia="">${title}</title>` }
                : {}),
        };
        const elements = Object.values(states)
            .reduce((carry, elements2) => carry.concat(elements2), [])
            .reduce((carry, element) => {
                if (element.indexOf("<") === -1) {
                    return carry;
                }
                if (element.indexOf("<title ") === 0) {
                    const title2 = element.match(
                        /(<title [^>]+>)(.*?)(<\/title>)/s,
                    );
                    carry.title = title2
                        ? `${title2[1]}${titleCallback(title2[2])}${title2[3]}`
                        : element;
                    return carry;
                }
...
var useInfiniteScrollData = (options) => {
    const getScrollPropFromCurrentPage = () => {
        ...
        throw new Error(
            `The page object does not contain a scroll prop named "${options.getPropName()}".`,
        );
    };
    const state = {
        component: null,
    ...
    const getRememberKey = () =>
        `inertia:infinite-scroll-data:${options.getPropName()}`;
    if (typeof window !== "undefined") {
        resetState();
        ...
        if (
            ...
            rememberedState.lastLoadedPage ===
                getScrollPropFromCurrentPage().currentPage
        ) {
    ...
    const removeEventListener = router.on("success", (event) => {
        if (
            state.component === event.detail.page.component &&
            getScrollPropFromCurrentPage().reset
        ) {
            resetState();
            options.onReset?.();
        }
    ...
    const fetchPage = (side, reloadOptions = {}) => {
        ...
        router.reload({
            ...
            headers: {
                [MERGE_INTENT_HEADER]:
                    side === "previous" ? "prepend" : "append",
                ...reloadOptions.headers,
            },
            onBefore: (visit) => {
                side === "next"
                    ? options.onBeforeNextRequest()
                    : options.onBeforePreviousRequest();
    ...
    const fetchPrevious = (reloadOptions) =>
        fetchPage("previous", reloadOptions);
...
var useInfiniteScrollElementManager = (options) => {
    ...
    const setupObservers = () => {
        ...
        itemsMutationObserver.observe(options.getItemsElement(), {
            childList: true,
        });
        itemsObserver = intersectionObservers.new((entry) =>
            options.onItemIntersected(entry.target),
        );
        const observerOptions = {
            root: options.getScrollableParent(),
            rootMargin: `${Math.max(1, options.getTriggerMargin())}px`,
        };
        startElementObserver = intersectionObservers.new(
            options.onPreviousTriggered,
            observerOptions,
        );
        endElementObserver = intersectionObservers.new(
            options.onNextTriggered,
            observerOptions,
        );
    };
    ...
    const elementIsUntagged = (element) =>
        !(INFINITE_SCROLL_PAGE_KEY in element.dataset) &&
        !(INFINITE_SCROLL_IGNORE_KEY in element.dataset);
        ...
        findUntaggedElements(options.getItemsElement()).forEach((element) => {
            if (elementIsUntagged(element)) {
                element.dataset[INFINITE_SCROLL_PAGE_KEY] =
                    loadedPage?.toString() || "1";
            }
            itemsObserver.observe(element);
        });
        rememberElements();
    };
    const getElementsRememberKey = () =>
        `inertia:infinite-scroll-elements:${options.getPropName()}`;
...
var useInfiniteScrollQueryString = (options) => {
    ...
    const onItemIntersected = debounce((itemElement) => {
        ...
        getElementsInViewportFromCollection(elements, itemElement).forEach(
            (element) => {
        ...
        const sortedPages = Array.from(pageMap.entries()).sort(
            (a, b) => b[1] - a[1],
        );
    ...
    const createCallbacks = () => {
        ...
        const captureScrollPosition = () => {
            ...
            if (visibleElements.length > 0) {
                ...
                const containerTop = scrollableContainer
                    ? containerRect.top
                    : 0;
...
function useInfiniteScroll(options) {
    ...
    const addScrollPreservationCallbacks = (reloadOptions) => {
        const { captureScrollPosition, restoreScrollPosition } =
            scrollPreservation.createCallbacks();
        const originalOnBeforeUpdate =
            reloadOptions.onBeforeUpdate || (() => {});
    ...
    const removeEventListener = router.on("success", () =>
        requestAnimationFrame(elementManager.refreshTriggers, 2),
    );
    return {
        dataManager,
        elementManager,
...
function isNamedLayouts(value, isComponent) {
    ...
    return Object.values(value).every(
        (v) =>
            isComponent(v) ||
            (Array.isArray(v) && isComponent(v[0])) ||
            (hasComponentKey(v) && isComponent(v.component)),
    );
}
...
function shouldNavigate(event) {
    ...
    return (
        !isContentEditableOrPrevented(event) &&
        (event.key === "Enter" || (isButton && event.key === " "))
    );
}
...
function handleStartEvent(event, delay) {
    ...
    document.addEventListener("inertia:finish", (e) => finish(e, timeout), {
        once: true,
    });
}
...
function resetInputElement(input, defaultValues) {
    ...
    switch (input.type.toLowerCase()) {
        ...
        default:
            input.value =
                defaultValues[0] !== null && defaultValues[0] !== void 0
...
function resetSelectElement(select, defaultValues) {
    ...
    const oldSelectedOptions = Array.from(select.selectedOptions).map(
        (opt) => opt.value,
    );
    if (select.multiple) {
        ...
        select.value =
            defaultValues[0] !== void 0 ? String(defaultValues[0]) : "";
    }
    const newSelectedOptions = Array.from(select.selectedOptions).map(
        (opt) => opt.value,
    );
    const hasChanged = select.multiple
        ? JSON.stringify(oldSelectedOptions.sort()) !==
          JSON.stringify(newSelectedOptions.sort())
...
function resetFormElement(element, defaultValues) {
    if (element.disabled) {
        if (element instanceof HTMLInputElement) {
            ...
            const oldSelectedOptions = Array.from(element.selectedOptions).map(
                (opt) => opt.value,
            );
            Array.from(element.options).forEach((option) => {
                option.selected = option.defaultSelected;
            });
            const newSelectedOptions = Array.from(element.selectedOptions).map(
                (opt) => opt.value,
            );
            return (
                JSON.stringify(oldSelectedOptions.sort()) !==
            ...
            if (node instanceof Element && isFormElement(node)) {
                if (
                    ...
                    const indexedDefaultValues =
                        defaultValues[index] !== void 0
                            ? [defaultValues[index]]
                            : [defaultValues[0] ?? null].filter(Boolean);
